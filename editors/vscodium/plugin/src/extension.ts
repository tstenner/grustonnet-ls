...
export async function activate(context: ExtensionContext): Promise<void> {
  ...
  context.subscriptions.push(
    ...
    commands.registerCommand('jsonnet.evalFile2', evalCommand(false)),
  );
}
...
function evalCommand(yaml: boolean, promptExpr = false) {
  return async () => {
    ...
    if (workspace.getConfiguration('jsonnet').get('languageServer.continuousEval') === false) {
      ...
      // Initial eval
      evalJsonnet(params, yaml, tempFile, true);
      channel.appendLine(`Watching eval...`);
...
function createTmpFile(yaml: any): string {
...
function getBinPath(): string {
  return workspace.getConfiguration("jsonnet").get("languageServer.pathToBinary");
}
...
async function startClient(): Promise<void> {
  ...
  const binPath: string = getBinPath();
  channel.appendLine(binPath);
  if (!binPath) {
    return;
  }
