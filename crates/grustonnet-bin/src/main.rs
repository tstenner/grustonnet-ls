...
async fn main() {
    ...
    // Windows does not support "execvp" (yes there are ways, but that is a task for later)
    #[cfg(unix)]
    {
        if std::env::var("GODEBUG").is_err() {
            ...
            let exe =
                std::env::current_exe().expect("Could not get path to the current executable");
